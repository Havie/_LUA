...
function core_object:load_mods(...)
	...
	for i = 1, arg.n do
		...
		for filename in string.gmatch(file_str, '([^,]+)') do
			local ok, load_err = pcall(function(filename) self:load_mod_script(filename) end, filename);
			
			if ok then
				ModLog("Mod [" .. tostring(filename) .. "] loaded successfully");
			else
				ModLog("Failed to load mod: [" .. tostring(filename) .. "], error is: " .. tostring(load_err));
				ModLog("Testing a pcall --")

				local test_file, require_err = pcall(require, filename)

				if test_file then
					ModLog("Loading mod file ["..tostring(filename).."] after pcall")
					setfenv(test_file, self:get_env())
					...
					ModLog("ERROR (pcall failed): ["..tostring(filename).."]")
					ModLog("\t"..tostring(require_err))
				end
				all_ok = false;
			end;
		end;
...
function core_object:event_callback(event_name, context)
	...
	--[[
	if event_name ~= "ScriptEventPreDeleteModelInterface" and event_name ~= "CampaignTimeTriggerEvent" and event_name ~= "ComponentLClickUp" then
...
function core_object:cli_listener_exists(listener_name)
    ...
    for i, listener in ipairs(self.cli_listeners) do
        if listener[i] == listener_name then
            return true;
        end;
    end;
