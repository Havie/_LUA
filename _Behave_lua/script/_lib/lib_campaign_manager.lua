...
function get_cm()
	if __campaign_manager then
		...
		ModLog("get_cm() called but no campaign manager created as yet");
	end;
end;
...
function campaign_manager:new(name)
	...
	if __campaign_manager then
		ModLog("WARNING: Cannot create more than one campaign manager!");
		return __campaign_manager;
	end;

	if not is_string(name) then
		ModLog("ERROR: Attempted to create campaign manager but no campaign name was supplied, or supplied name [" .. tostring(name) .. "] is not a string!");
		return false;
	end;
...
function campaign_manager:require_path_to_local_campaign_faction_folder()
	...
	if not local_faction then
		ModLog("ERROR: require_path_to_local_campaign_faction_folder() called but no local faction could be found - has it been called too early during the load sequence, or during an autotest?");
		return false;
	end;
...
function campaign_manager:require_path_to_specific_campaign_faction_folder(campaign_folder, faction_key)
	...
	if not is_string(campaign_folder) then
		ModLog("ERROR: require_path_to_specific_campaign_faction_folder() called but supplied campaign folder [" .. tostring(campaign_folder) .. "] is not a string.");
		return false;
	end;

	if not is_string(faction_key) then
		ModLog("ERROR: require_path_to_specific_campaign_faction_folder() called but supplied faction [" .. tostring(faction_key) .. "] is not a string.");
		return false;
	end;
...
function campaign_manager:add_pre_first_tick_callback(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_pre_first_tick_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_first_tick_callback(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_first_tick_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_first_tick_callback_sp_new(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_first_tick_callback_sp_new() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_first_tick_callback_mp_new(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_first_tick_callback_mp_new() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_first_tick_callback_new(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_first_tick_callback_new() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_saving_game_callback(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_saving_game_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:register_post_saving_game_callback(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: register_post_saving_game_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_loading_game_callback(callback)
	if not is_function(callback) then
		ModLog(self.name .. " ERROR: add_loading_game_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:loading_game(context)
	...
	if not self:is_new_game() then
		...
		pending_battle_cache.cached_battle =  cm:load_named_value("pending_battle_cache_cached_battle", pending_battle_cache.cached_last_battle);
...
function campaign_manager:save_named_value(name, value)
	...
	if not is_string(name) then
		ModLog("ERROR: save_named_value() called but supplied value name [" .. tostring(name) .. "] is not a string");
		return false;
	end;
	...
	elseif is_table(value) then
		...
		ModLog("ERROR: save_named_value() called with name [" .. name .. "] but value type [" .. type(value) .. " is unsupported");
	end;
end;
...
function campaign_manager:load_named_value(name, default)
	...
	if not is_string(name) then
		ModLog("ERROR: load_named_value() called but supplied value name [" .. tostring(name) .. "] is not a string");
		return false;
	end;
	...
	elseif is_table(default) then
		...
		ModLog("ERROR: load_named_value() called with name [" .. name .. "] but type [" .. type(default) .. "] of supplied default value is not supported");
	end;
...
function campaign_manager:set_saved_value(key, value, ...)
	if not is_string(key) then
		ModLog("ERROR: set_saved_value() called but specified key [" .. tostring(key) .. "] is not a string");
		return false;
	end;

	if not (is_string(value) or is_number(value) or is_boolean(value)) then
		ModLog("ERROR: set_saved_value("..tostring(key)..") called but specified value [" .. tostring(value) .. "] is not a string, number or boolean");
		return false;
	end;
	
	for i = 1, arg.n do
		if not is_string(arg[i]) then
			ModLog("ERROR: set_saved_value() called but container name [" .. i .. "] is not a string, its value is [" .. tostring(arg[i]) .. "]")
			return false;
		end;
	end;
	...
	for i = 1, arg.n do
		if is_nil(saved_values_at_level[arg[i]]) then
			...
			if not is_table(saved_values_at_level) then
				ModLog("ERROR: set_saved_value() called but container [" .. i .. "] with name [" .. arg[i] .. "] is not a table, its value is [" .. tostring(saved_values_at_level) .. "]");
				return false;
			end;
		end;
	end;
...
function campaign_manager:get_saved_value( key, ... )
	if not is_string(key) then
		ModLog("ERROR: get_saved_value() called but specified key [" .. tostring(key) .. "] is not a string");
		return nil;
	end;
	
	for i = 1, arg.n do
		if not is_string(arg[i]) then
			ModLog("ERROR: get_saved_value() called but container name [" .. i .. "] is not a string, its value is [" .. tostring(arg[i]) .. "]");
	...
	for i = 1, arg.n do
		if is_nil(saved_values_at_level[arg[i]]) then
			ModLog("ERROR: get_saved_value() called but container [" .. i .. "] with name [" .. arg[i] .. "] is does not exist, so no data is stored. You should use cm:saved_value_exists() to test before calling this.");
			return nil;
		else
			saved_values_at_level = saved_values_at_level[arg[i]];
			
			if not is_table(saved_values_at_level) then
				ModLog("ERROR: get_saved_value() called but container [" .. i .. "] with name [" .. arg[i] .. "] is not a table, its value is [" .. tostring(saved_values_at_level) .. "]");
				return nil;
			end;
		end;
	end;
...
function campaign_manager:load_values_from_string(str)
	...
	-- Legacy save load system.
		...
		if not is_string(str) then
			ModLog("ERROR: load_values_from_string() called but supplied string [" .. tostring(str) .. "] is not a string");
			return false;
		end;
		...
		while true do
			...
			if not next_separator then
				ModLog("ERROR: load_values_from_string() called but supplied str is malformed: " .. str);
				return false;
			end;
			...
			if not num_value_length then
				ModLog("ERROR: load_values_from_string() called, but retrieved value_length [" .. tostring(value_length) .. "] could not be converted to a number in string: " .. str);
				return false;
			end;
			...
			elseif value_type == "number" then
				...
				if not value_number then
					ModLog("ERROR: load_values_from_string() called, but couldn't convert loaded numeric value [" .. value .. "] to a number in string: " .. str);
					return false;
				else
					value = value_number;
				end;
			elseif value_type ~= "string" then
				ModLog("ERROR: load_values_from_string() called, but couldn't recognise supplied value type [" .. tostring(value_type) .. "] in string: " .. str);
				return false;
			end;
...
function campaign_manager:is_multiplayer()
	if not self.game_is_created then
		ModLog(self.name .. " ERROR: is_multiplayer() called before game has been created!");
		return false;
	end;
...
function campaign_manager:is_new_game()
	if not self.game_loaded then
		ModLog(self.name .. " WARNING: is_new_game() called before the game has loaded, this call should happen later in the loading process. Returning false.");
		return false;
	end;
...
function campaign_manager:current_context()
	if not self.context then
		ModLog("ERROR: current_context() failed. No context is available. Last event triggered is " .. self:get_last_triggered_event_name());
		return nil;
	end;
...
function campaign_manager:can_modify(silent)
	if not self.have_modify_interface then
		if not silent then
			ModLog("ERROR: can_modify() failed - no model interface is present. Last event triggered is " .. self:get_last_triggered_event_name(), 1);
		end;
		return false;
	end;
	return true;
...
function campaign_manager:modify_scripting()
	if not self.have_modify_interface then
		ModLog("ERROR: modify_scripting() called but we don't have access to modify the model");
		return false;
	end;
...
function campaign_manager:faction_exists(faction_key)
	if not is_string(faction_key) then
		ModLog("ERROR: faction_exists() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:query_faction(faction_key, assert_on_failure)
	if not is_string(faction_key) then
		ModLog("ERROR: query_faction() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
	...
	elseif assert_on_failure then
		ModLog("ERROR: query_faction() could find no faction with key [" .. tostring(faction_key) .. "]");
	end;
end;
...
function campaign_manager:query_faction_by_cqi(cqi, assert_on_failure)
	...
	if not is_number(cqi) then
		ModLog("ERROR: query_faction_by_cqi() called but supplied faction cqi [" .. tostring(cqi) .. "] is not a number");
		return false;
	end;
	...
	elseif assert_on_failure then
		...
		ModLog("ERROR: query_faction() could find no faction with key [" .. tostring(cqi) .. "]");
	end;
end;
...
function campaign_manager:query_region(region_key, exists)
	if not is_string(region_key) then
		ModLog("ERROR: query_region() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:query_character(cqi)
	...
	if not is_number(cqi) then
		ModLog("query_character() called but supplied cqi [" .. tostring(cqi) .. "] is not a number or a string that converts to a number");
		return false;
	end;
...
function campaign_manager:wait_for_model_sp(callback)
	...
	if self:is_multiplayer() then
		ModLog("ERROR: wait_for_model_sp() called but this is a multiplayer game");
		return false;
	end;
	...
	if not self.modify_model_event_created then
		...
		if not context then
			ModLog("ERROR: wait_for_model_sp() called and we have no stored context - how can this be? - "  .. tostring(context.string));
			return false;
			
		elseif not context:can_request_model_callback() then
			ModLog("ERROR: wait_for_model_sp() called and we cannot currently modify or create a modify model event - how can this be? - " .. tostring(context.string));
			return false;
		end;
...
function campaign_manager:wait_for_model_with_id(id, callback, persistent)
	...
	if id and not is_string(id) then
		ModLog("ERROR: wait_for_model_with_id() called but supplied callback id [" .. tostring(id) .. "] is not a string or nil");
		return false;
	end;

	if not is_function(callback) then
		ModLog("ERROR: wait_for_model_with_id() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
	...
	if self:is_multiplayer() and not self:can_modify(true) then
		ModLog("ERROR: wait_for_model_with_id() called in multiplayer mode, but no modify interface is currently registered. This means that the machines in the game will not be in sync.");
		return false;
	end;
...
function campaign_manager:trigger_callback_request(id)
	if not is_string(id) then
		ModLog("ERROR: trigger_callback_request() called but supplied id [" .. tostring(id) .. "] is not a string");
		return false;
	end;
	...
	return self.context:create_model_callback_request(id);
end;
...
function campaign_manager:load_faction_script(scriptname, single_player_only)
	...
	ModLog("ERROR: Tried to load faction script " .. scriptname .. " without success - either the script is not present or it is not valid. See error below");
	output("*************");
...
function campaign_manager:find_local_faction(context)
	...
	ModLog("WARNING: find_local_faction() called but couldn't find a local faction - this should only happen in autoruns");
	return false;
end;
...
function campaign_manager:get_local_faction(force)
	if not self.game_is_created then
		ModLog(self.name .. " ERROR: get_local_faction() called before game has been created!");
		return false;
	end;
	
	if self:is_multiplayer() and not force then
		ModLog(self.name .. " ERROR: get_local_faction() called but this is a multiplayer game, reconsider or force this usage");
		return false;
	end;
...
function campaign_manager:get_human_factions()
	if not self.game_is_created then
		ModLog(self.name .. " ERROR: get_human_faction() called before game has been created!");
		return false;
	end;
...
function campaign_manager:load_local_faction_scripts(name_appellation)
	...
	if name_appellation then
		if not is_string(name_appellation) then
			ModLog("ERROR: load_local_faction_scripts() called but supplied name appellation [" .. tostring(name_appellation) .. "] is not a string");
			return false;
		end;
	else
		name_appellation = "";
	...
	self:load_specified_faction_scripts(local_faction, name_appellation);
end;


function campaign_manager:load_specified_faction_scripts(faction_key, name_appellation)
	if not is_string(faction_key) then
		ModLog("ERROR: load_specified_faction_scripts() called but supplied faction_key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;

	if name_appellation then
		if not is_string(name_appellation) then
			ModLog("ERROR: load_specified_faction_scripts() called but supplied name appellation [" .. tostring(name_appellation) .. "] is not a string");
			return false;
		end;
	else
		name_appellation = "";
...
function campaign_manager:load_faction_scripts_for_both_mp_factions(name_appellation)
	if name_appellation then
		if not is_string(name_appellation) then
			ModLog("ERROR: load_faction_scripts_for_both_mp_factions() called but supplied name appellation [" .. tostring(name_appellation) .. "] is not a string");
			return false;
		end;
	else
		name_appellation = "";
	...
	for i, faction in ipairs(human_factions) do
		self:load_specified_faction_scripts(faction, name_appellation);
	end;
	dec_tab();
end;
...
function campaign_manager:check_valid_camera_waypoint(waypoint)
	if not is_table(waypoint) then
		ModLog("ERROR: check_valid_camera_waypoint() called but supplied waypoint [" .. tostring(waypoint) .. "] is not a table");
		return false;
	end;
	
	for i = 1, 5 do
		if not is_number(waypoint[i]) then
			ModLog("ERROR: check_valid_camera_waypoint() called but index [" .. i .. "] of supplied waypoint is not a number but is [" .. tostring(waypoint[i]) .. "]");
			return false;
		end;
	end;
	...
	if #waypoint == 6 then
		if not is_number(waypoint[6]) then
			ModLog("ERROR: check_valid_camera_waypoint() called but index [" .. 6 .. "] of supplied waypoint is not a number but is [" .. tostring(waypoint[6]) .. "]");
			return false;
		end;
	end;
...
function campaign_manager:scroll_camera_with_direction(valid_endpoint, t, ...)
	...
	if not is_number(t) or t < 0 then
		ModLog("ERROR: scroll_camera_with_direction() called but supplied duration [" .. tostring(t) .. "] is not a positive number");
		return false;
	end;
...
function campaign_manager:scroll_camera_from_current(t, correct_endpoint, ...)
	...
	if not is_number(t) or t <= 0 then
		ModLog("ERROR: scroll_camera_from_current() called but supplied duration [" .. tostring(t) .. "] is not a number");
		return false;
	end;
...
function campaign_manager:scroll_camera_with_cutscene(t, end_callback, ...)
	...
	-- check our parameters
	if not is_number(t) or t <= 0 then
		ModLog("ERROR: scroll_camera_with_cutscene() called but supplied duration [" .. tostring(t) .. "] is not a number");
		return false;
	end;
	
	if not is_function(end_callback) and not is_nil(end_callback) then
		ModLog("ERROR: scroll_camera_with_cutscene() called but supplied end_callback [" .. tostring(end_callback) .. "] is not a function or nil");
		return false;
	end;
...
function campaign_manager:cut_and_scroll_camera_with_cutscene(t, end_callback, ...)
	...
	-- check our parameters
	if not is_number(t) or t <=0 then
		ModLog("ERROR: cut_and_scroll_camera_with_cutscene() called but supplied duration [" .. tostring(t) .. "] is not a number");
		return false;
	end;
	
	if not is_function(end_callback) and not is_nil(end_callback) then
		ModLog("ERROR: cut_and_scroll_camera_with_cutscene() called but supplied end_callback [" .. tostring(end_callback) .. "] is not a function or nil");
		return false;
	end;
	
	if arg.n < 2 then
		ModLog("ERROR: cut_and_scroll_camera_with_cutscene() called but less than two camera positions given");
		return false;
	end;
...
function campaign_manager:scroll_camera_with_cutscene_to_settlement(t, end_callback, region_key)
	if not is_string(region_key) then
		ModLog("ERROR: scroll_camera_with_cutscene_to_settlement() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return false;
	end;
	...
	if not region then
		ModLog("ERROR: scroll_camera_with_cutscene_to_settlement() called but region with supplied key [" .. region_key .. "] could not be found");
		return false;
	end;
...
function campaign_manager:scroll_camera_with_cutscene_to_character(t, end_callback, char_cqi)
	if not is_number(char_cqi) then
		ModLog("ERROR: scroll_camera_with_cutscene_to_character() called but supplied character cqi [" .. tostring(char_cqi) .. "] is not a number");
		return false;
	end;
	...
	if not character then
		ModLog("ERROR: scroll_camera_with_cutscene_to_character() called but no character with cqi [" .. char_cqi .. "] could be found");
		return false;
	end;
	...
	self:scroll_camera_with_cutscene(
		t, 
		end_callback,
		{targ_x, targ_y, 8, r, 10}			-- customise constants to suit
	);
end;
...
function campaign_manager:position_camera_at_primary_military_force(faction_name)
	if not is_string(faction_name) then
		ModLog("ERROR: position_camera_at_primary_military_force() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
	...
	if not query_faction then
		ModLog("ERROR: position_camera_at_primary_military_force() called but no faction with name [" .. faction_name .. "] could be found");
		return false;
	end;
	
	if not query_faction:has_faction_leader() then
		ModLog("ERROR: position_camera_at_primary_military_force() called but no faction leader could be found for faction [" .. faction_name .. "]");
		return false;
	end;
	...
	if faction_leader:has_military_force() then
		...
		if mf_list_item_0:has_general() then
			...
			ModLog("ERROR: position_camera_at_primary_military_force() called but no military force for faction [" .. faction_name .. "] could be found on the map");
		end;
	end
...
function campaign_manager:show_advice(key, progress_button, highlight, playtime, callback, delay)
	...
	if not is_string(key) then
		ModLog("ERROR: show_advice() called but supplied key [" .. tostring(key) .. "] is not a string");
		return false;
	end;
	...
	if not is_function(callback) then
		ModLog("show_advice() called but supplied callback [" .. tostring(callback) .. "] is not a function. Key is " .. tostring(key));
		return;
	end;
...
function campaign_manager:set_show_advice_text(value)
	...
	if not uic_advice_text then
		ModLog("ERROR: set_show_advice_text() couldn't find uic_advice_text");
		return false;
	end;
...
function campaign_manager:register_pre_dismiss_advice_callback(callback)
	if not is_function(callback) then
		ModLog("ERROR: register_pre_dismiss_advice_callback() called but supplied callback [" .. tostring(callback) .."] is not a function");
		return false;
	end;
...
function campaign_manager:dismiss_advice()
	if not core:is_ui_created() then
		ModLog("ERROR: dismiss_advice() called but ui not created");
		return false;
	end;
...
function campaign_manager:progress_on_advice_dismissed(callback, delay, highlight_on_finish)
	if not is_function(callback) then
		ModLog("ERROR: progress_on_advice_dismissed() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:progress_on_advice_finished(callback, delay, playtime, use_os_clock)
	if not is_function(callback) then
		ModLog("ERROR: progress_on_advice_finished() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:progress_on_events_dismissed(name, callback, delay, wait_between_tests)
	if cm:is_multiplayer() then
		ModLog("ERROR: progress_on_events_dismissed() called in multiplayer mode");
		return false;
	end;

	if not is_string(name) then
		ModLog("ERROR: progress_on_events_dismissed() called but supplied name [" .. tostring(name) .. "] is not a string");
		return false;
	end;
	
	if not is_function(callback) then
		ModLog("ERROR: progress_on_events_dismissed() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
	...
	if not is_number(delay) or delay < 0 then
		ModLog("ERROR: progress_on_events_dismissed() called but supplied delay [" .. tostring(delay) .. "] is not a positive number or nil");
		return false;
	end;
...
function campaign_manager:progress_on_uicomponent_animation(name, uicomponent, callback, delay, animation_id)
	if not is_string(name) then
		ModLog("ERROR: progress_on_uicomponent_animation() called but supplied name [" .. tostring(name) .. "] is not a string");
		return false;
	end;
	
	if not is_uicomponent(uicomponent) then
		ModLog("ERROR: progress_on_uicomponent_animation() called but supplied uicomponent [" .. tostring(uicomponent) .. "] is not a uicomponent");
		return false;
	end;
	
	if not is_function(callback) then
		ModLog("ERROR: progress_on_uicomponent_animation() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
	...
	if not is_number(delay) or delay < 0 then
		ModLog("ERROR: progress_on_uicomponent_animation() called but supplied delay [" .. tostring(delay) .. "] is not a positive number or nil");
		return false;
	end;
	...
	if not is_string(animation_id) then
		ModLog("ERROR: progress_on_uicomponent_animation() called but supplied animation id [" .. tostring(animation_id) .. "] is not a string or nil");
		return false;
	end;
...
function campaign_manager:start_intro_cutscene_on_loading_screen_dismissed(callback, fade_in_duration)
	if not is_function(callback) then
		ModLog("ERROR: start_intro_cutscene_on_loading_screen_dismissed() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
	...
	if not is_number(fade_in_duration) or fade_in_duration < 0 then
		ModLog("ERROR: start_intro_cutscene_on_loading_screen_dismissed() called but supplied fade in duration [" .. tostring(fade_in_duration) .. "] is not a positive number or nil");
		return false;
	end;

	if not self:can_modify() then
		ModLog("ERROR: start_intro_cutscene_on_loading_screen_dismissed() called but we have no modify interface.");
		return false;
	end;
...
function campaign_manager:progress_on_mission_accepted(callback, delay, should_lock)
	if not is_function(callback) then
		ModLog("ERROR: progress_on_mission_accepted() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:progress_on_battle_completed(name, callback, delay)
	...
	if not is_string(name) then
		ModLog("ERROR: progress_on_battle_completed() called but supplied name [" .. tostring(name) .. "] is not a string");
		return false;
	end;

	if not is_function(callback) then
		ModLog("ERROR: progress_on_battle_completed() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:cancel_progress_on_battle_completed(name)
	if not is_string(name) then
		ModLog("ERROR: cancel_progress_on_battle_completed() called but supplied name [" .. tostring(name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:progress_on_camera_movement_finished(callback, delay)
	...
	if not is_function(callback) then
		ModLog("ERROR: progress_on_camera_movement_finished() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:progress_on_faction_turn_start(faction_name, callback)
	if not is_string(faction_name) then
		ModLog("ERROR: progress_on_faction_turn_start() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;

	if not is_function(callback) then
		ModLog("ERROR: progress_on_faction_turn_start() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:set_default_turn_start_callback(callback)
	if not is_function(callback) and not is_nil(callback) then
		ModLog("campaign_manager ERROR:: set_default_turn_start_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function or nil");
		return false;
	end;
...
function campaign_manager:add_turn_start_callback(new_name, new_condition, new_callback, new_should_repeat)
	if not is_string(new_name) then
		ModLog("campaign_manager ERROR: add_turn_start_callback() called but supplied name [" .. tostring(new_name) .. "] is not a string");
		return false;
	end;
	
	if not is_function(new_condition) then
		ModLog("campaign_manager ERROR: add_turn_start_callback() called but supplied condition [" .. tostring(new_condition) .. "] is not a function");
		return false;
	end;
	
	if not is_function(new_callback) then
		ModLog("campaign_manager ERROR: add_turn_start_callback() called but supplied callback [" .. tostring(new_callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_turn_start_callback_for_faction(name, faction_name, callback, should_repeat)
	if not is_string(faction_name) then
		ModLog("campaign_manager ERROR: add_turn_start_callback_for_faction() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:set_default_turn_end_callback(callback)
	if not is_function(callback) and not is_nil(callback) then
		ModLog("campaign_manager ERROR:: set_default_turn_end_callback() called but supplied callback [" .. tostring(callback) .. "] is not a function or nil");
		return false;
	end;
...
function campaign_manager:add_turn_end_callback(new_name, new_condition, new_callback, new_should_repeat)
	if not is_string(new_name) then
		ModLog("campaign_manager ERROR: add_turn_end_callback() called but supplied name [" .. tostring(new_name) .. "] is not a string");
		return false;
	end;
	
	if not is_function(new_condition) then
		ModLog("campaign_manager ERROR: add_turn_end_callback() called but supplied condition [" .. tostring(new_condition) .. "] is not a function");
		return false;
	end;
	
	if not is_function(new_callback) then
		ModLog("campaign_manager ERROR: add_turn_end_callback() called but supplied callback [" .. tostring(new_callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:add_turn_end_callback_for_faction(name, faction_name, callback, should_repeat)
	if not is_string(faction_name) then
		ModLog("campaign_manager ERROR: add_turn_end_callback_for_faction() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:create_force(faction_key, unit_list, region_key, x, y, id, exclude_named_characters, success_callback, starting_health_percentage)
	starting_health_percentage = starting_health_percentage or 100;

	if not is_string(faction_key) then
		ModLog("ERROR: create_force() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: create_force() called but no faction with supplied key [" .. faction_key .. "] was found");
		return;
	end;
	...
	--[[
	if unit_list == "" then
		ModLog("ERROR: create_force() called but supplied unit list [" .. tostring(unit_list) .. "] is an empty string");
		return;
	end;]]--
	
	if not is_string(region_key) then
		ModLog("ERROR: create_force() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return;
	end;
	
	if not is_number(x) or x < 0 then
		ModLog("ERROR: create_force() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a positive number");
		return;
	end;
	
	if not is_number(y) or y < 0 then
		ModLog("ERROR: create_force() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a positive number");
		return;
	end;
	
	if not is_string(id) then
		ModLog("ERROR: create_force() called but supplied id [" .. tostring(id) .. "] is not a string");
		return;
	end;
	
	if not is_boolean(exclude_named_characters) then
		ModLog("ERROR: create_force() called but supplied exclude named characters switch [" .. tostring(exclude_named_characters) .. "] is not a boolean value");	
		return;
	end;
	
	if not is_function(success_callback) and not is_nil(success_callback) then
		ModLog("ERROR: create_force() called but supplied success callback [" .. tostring(success_callback) .. "] is not a function or nil");
		return;
	end;
	...
	output("starting_health_percentage: " .. tostring(starting_health_percentage));
	
	dec_tab();
	
	-- make the call to create the force
	self:modify_faction(faction_key):create_force(unit_list, region_key, x, y, id, exclude_named_characters, starting_health_percentage);
...
function campaign_manager:create_force_with_general(faction_key, unit_list, region_key, x, y, agent_type, agent_subtype, character_template_key, id, make_faction_leader, success_callback, starting_health_percentage)
	starting_health_percentage = starting_health_percentage or 100;

	if not is_string(faction_key) then
		ModLog("ERROR: create_force_with_general() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: create_force_with_general() called but no faction with supplied key [" .. faction_key .. "] was found");
		return;
	end;
	...
	if not is_string(region_key) then
		ModLog("ERROR: create_force_with_general() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return;
	end;
	
	if not is_number(x) or x < 0 then
		ModLog("ERROR: create_force_with_general() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a positive number");
		return;
	end;
	
	if not is_number(y) or y < 0 then
		ModLog("ERROR: create_force_with_general() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a positive number");
		return;
	end;
	
	if not is_string(agent_type) then
		ModLog("ERROR: create_force_with_general() called but supplied agent_type [" .. tostring(agent_type) .. "] is not a string");
		return;
	end;
	
	if not is_string(agent_subtype) then
		ModLog("ERROR: create_force_with_general() called but supplied agent_subtype [" .. tostring(agent_subtype) .. "] is not a string");
		return;
	end;
	
	if not is_string(character_template_key) then
		ModLog("ERROR: create_force_with_general() called but supplied character template key [" .. tostring(character_template_key) .. "] is not a string");
		return;
	end;
	
	if not is_string(id) then
		ModLog("ERROR: create_force_with_general() called but supplied id [" .. tostring(id) .. "] is not a string");
		return;
	end;

	if not is_boolean(make_faction_leader) then
		ModLog("ERROR: create_force() called but supplied make faction leader switch [" .. tostring(make_faction_leader) .. "] is not a boolean value");
		return;
	end;
	
	if not is_function(success_callback) and not is_nil(success_callback) then
		ModLog("ERROR: create_force_with_general() called but supplied success callback [" .. tostring(success_callback) .. "] is not a function or nil");
		return;
	end;
	...
	-- make the call to create the force
	self:modify_faction(faction_key):create_force_with_general(unit_list, region_key, x, y, agent_type, agent_subtype, character_template_key, id, make_faction_leader, starting_health_percentage);
end;
...
function campaign_manager:create_force_with_existing_general(char_cqi, faction_key, unit_list, region_key, x, y, id, success_callback, starting_health_percentage)
	starting_health_percentage = starting_health_percentage or 100;

	if not is_number(char_cqi) then
		ModLog("ERROR: create_force_with_existing_general() called but supplied character cqi [" .. tostring(char_cqi) .. "] is not a number");
		return;
	end;
	...
	if not modify_character or modify_character:is_null_interface() then
		ModLog("ERROR: create_force_with_existing_general() called but supplied character string [" .. tostring(char_cqi) .. "] is not a valid character");
		return;
	end;
	
	if not is_string(faction_key) then
		ModLog("ERROR: create_force_with_existing_general() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: create_force_with_existing_general() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return;
	end;
	...
	if not is_string(region_key) then
		ModLog("ERROR: create_force_with_existing_general() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return;
	end;
	
	if not is_number(x) or x < 0 then
		ModLog("ERROR: create_force_with_existing_general() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a positive number");
		return;
	end;
	
	if not is_number(y) or y < 0 then
		ModLog("ERROR: create_force_with_existing_general() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a positive number");
		return;
	end;
	
	if not is_string(id) then
		ModLog("ERROR: create_force_with_existing_general() called but supplied id [" .. tostring(id) .. "] is not a string");
		return;
	end;
	
	if not is_function(success_callback) and not is_nil(success_callback) then
		ModLog("ERROR: create_force_with_existing_general() called but supplied success callback [" .. tostring(success_callback) .. "] is not a function or nil");
		return;
	end;
	...
	-- make the call to create the force
	modify_character:create_force(region_key, unit_list, x, y, id, starting_health_percentage);
...
function campaign_manager:create_agent(faction_key, agent_key, subtype_key, x, y, id, character_template_key, success_callback)
	if not is_string(faction_key) then
		ModLog("ERROR: create_agent() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not is_string(agent_key) then
		ModLog("ERROR: create_agent() called but supplied agent key [" .. tostring(agent_key) .. "] is not a string");
		return;
	end;
	
	if not is_string(subtype_key) then
		ModLog("ERROR: create_agent() called but supplied agent subtype key [" .. tostring(subtype_key) .. "] is not a string");
		return;
	end;
	
	if not is_number(x) or x < 0 then
		ModLog("ERROR: create_agent() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a positive number");
		return;
	end;
	
	if not is_number(y) or y < 0 then
		ModLog("ERROR: create_agent() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a positive number");
		return;
	end;
	
	if not is_string(id) then
		ModLog("ERROR: create_agent() called but supplied id [" .. tostring(id) .. "] is not a string");
		return;
	end;
	
	if not is_string(character_template_key) then
		ModLog("ERROR: create_agent() called but supplied character template key [" .. tostring(character_template_key) .. "] is not a string");
		return;
	end;
	
	if not is_function(success_callback) and not is_nil(success_callback) then
		ModLog("ERROR: create_agent() called but supplied success callback [" .. tostring(success_callback) .. "] is not a function or nil");
		return;
	end;
...
function campaign_manager:apply_effect_bundle(bundle_key, faction_key, turns)
	if not is_string(bundle_key) then
		ModLog("ERROR: apply_effect_bundle() called but supplied bundle key [" .. tostring(bundle_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(faction_key) then
		ModLog("ERROR: apply_effect_bundle() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: apply_effect_bundle() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
	
	if not is_number(turns) then
		ModLog("ERROR: apply_effect_bundle() called but supplied turn count [" .. tostring(turns) .. "] is not a number");
		return false;
	end;
...
function campaign_manager:remove_effect_bundle(bundle_key, faction_key)
	if not is_string(bundle_key) then
		ModLog("ERROR: remove_effect_bundle() called but supplied bundle key [" .. tostring(bundle_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(faction_key) then
		ModLog("ERROR: remove_effect_bundle() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: remove_effect_bundle() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:apply_effect_bundle_to_region(bundle_key, region_key, turns)
	if not is_string(bundle_key) then
		ModLog("ERROR: apply_effect_bundle_to_region() called but supplied bundle key [" .. tostring(bundle_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(region_key) then
		ModLog("ERROR: apply_effect_bundle_to_region() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return false;
	end;
	
	if not self:region_exists(region_key) then
		ModLog("ERROR: apply_effect_bundle_to_region() called but no region with supplied key [" .. tostring(region_key) .. "] could be found");
		return false;
	end;
	
	if not is_number(turns) then
		ModLog("ERROR: apply_effect_bundle_to_region() called but supplied turn count [" .. tostring(turns) .. "] is not a number");
		return false;
	end;
...
function campaign_manager:remove_effect_bundle_from_region(bundle_key, region_key)
	if not is_string(bundle_key) then
		ModLog("ERROR: remove_effect_bundle_from_region() called but supplied bundle key [" .. tostring(bundle_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(region_key) then
		ModLog("ERROR: remove_effect_bundle_from_region() called but supplied region key [" .. tostring(region_key) .. "] is not a string");
		return false;
	end;
	
	if not self:region_exists(region_key) then
		ModLog("ERROR: remove_effect_bundle_from_region() called but no region with supplied key [" .. tostring(region_key) .. "] could be found");
		return false;
	end;
...
function campaign_manager:log_to_dis(x, y)
	if not is_number(x) or x < 0 then
		ModLog("ERROR: log_to_dis() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a positive number");
		return;
	end;
	
	if not is_number(y) or y < 0 then
		ModLog("ERROR: log_to_dis() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a positive number");
		return;
	end;
...
function campaign_manager:random_number(max_num, min_num)
	...
	if not is_number(max_num) or math.floor(max_num) < max_num then
		ModLog("random_number ERROR: supplied max number [" .. tostring(max_num) .. "] is not a valid integer");
		return 0;
	end;
...
function campaign_manager:roll_random_chance( chance, is_debug )
	...
	if not self:can_modify() then
		ModLog( "Rolling random chance while no modify interface is present. Will return false." );
		return false;
	end;
...
function campaign_manager:restrict_units_for_faction(faction_key, unit_list, value)	
	if not is_string(faction_key) then
		ModLog("ERROR: restrict_units_for_faction() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not is_table(unit_list) then
		ModLog("ERROR: restrict_units_for_faction() called but supplied unit_list [" .. tostring(unit_list) .. "] is not a table");
		return;
	end;
	...
	if not modify_faction then
		ModLog("ERROR: restrict_units_for_faction() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:restrict_buildings_for_faction(faction_key, building_list, value)
	if not is_string(faction_key) then
		ModLog("ERROR: restrict_buildings_for_faction() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not is_table(building_list) then
		ModLog("ERROR: restrict_buildings_for_faction() called but supplied building_list [" .. tostring(building_list) .. "] is not a table");
		return;
	end;
	...
	if not modify_faction then
		ModLog("ERROR: restrict_buildings_for_faction() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:restrict_technologies_for_faction(faction_key, tech_list, value)
	if not is_string(faction_key) then
		ModLog("ERROR: restrict_technologies_for_faction() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return;
	end;
	
	if not is_table(tech_list) then
		ModLog("ERROR: restrict_technologies_for_faction() called but supplied tech_list [" .. tostring(tech_list) .. "] is not a table");
		return;
	end;
	...
	if not modify_faction then
		ModLog("ERROR: restrict_technologies_for_faction() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:generated_diplomacy_bitmask(str)
	if not is_string(str) then
		ModLog("ERROR: generate_diplomacy_bitmask() called but supplied diplomacy string [" .. tostring(str) .. "] is not a string");
		return 0;
	end;
	...
	for i = 1, #tokens do
		...
		if current_token == "all" then
			...
			ModLog("WARNING: generate_diplomacy_bitmask() was given a string [" .. str .. "] containing token [" .. current_token .. "] with other tokens - this token can only be used on its own, ignoring");
		else		
			local current_token_value = self.diplomacy_types[current_token];		
			if not current_token_value then
				ModLog("WARNING: generate_diplomacy_bitmask() was given a string [" .. str .. "] containing unrecognised token [" .. current_token .. "], ignoring");
			else
...
function campaign_manager:move_npc_army(char_cqi, log_x, log_y, should_replenish, allow_movement_afterwards, success_callback, fail_callback)
	...
	if not is_number(char_cqi) and not is_string(char_cqi) then
		ModLog("move_npc_army ERROR: cqi provided [" .. tostring(char_cqi) .. "] is not a number or string");
		return false;
	end;
		
	if not is_number(log_x) or log_x < 0 then
		ModLog("move_npc_army ERROR: supplied logical x co-ordinate [" .. tostring(log_x) .. "] is not a positive number");
		return false;
	end;
	
	if not is_number(log_y) or log_y < 0 then
		ModLog("move_npc_army ERROR: supplied logical y co-ordinate [" .. tostring(log_x) .. "] is not a positive number");
		return false;
	end;
	
	if not is_function(success_callback) and not is_nil(success_callback) then
		ModLog("move_npc_army ERROR: supplied success callback [" .. tostring(success_callback) .. "] is not a function or nil");
		return false;
	end;
	
	if not is_function(fail_callback) and not is_nil(fail_callback) then
		ModLog("move_npc_army ERROR: supplied failure callback [" .. tostring(fail_callback) .. "] is not a function or nil");
		return false;
	end;
...
function campaign_manager:move_npc_army_halted(char_cqi, log_x, log_y, should_replenish, allow_movement_afterwards, success_callback, fail_callback, trigger_name)
	...
	if not character then
		ModLog("ERROR: move_npc_army_halted() called but couldn't find a character with cqi [" .. tostring(char_cqi) .."]");
		return false;
	end;
...
function campaign_manager:is_character_moving(char_cqi, is_moving_callback, is_not_moving_callback)
	...
	if not is_number(char_cqi) then
		ModLog("ERROR: is_character_moving() called but supplied cqi [" .. tostring(char_cqi) .. "] is not a number");
		return false;
	end;
	...
	if not cached_char then
		ModLog("ERROR: is_character_moving() called but couldn't find char with cqi of [" .. char_cqi .. "]");
		return false;
	end;
	...
	self:os_clock_callback(
		function()
			...
			if not current_char then
				-- ModLog("WARNING: is_character_moving_action() called but couldn't find char with cqi of [" .. char_cqi .. "] after movement - did it die?");
				return false;
			end;
...
function campaign_manager:notify_on_character_halt(char_cqi, callback, must_move_first)
	if not is_function(callback) then
		ModLog("ERROR: notify_on_character_halt() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:notify_on_character_movement(process_name, char_cqi, callback, land_only)
	if not is_string(process_name) then
		ModLog("ERROR: notify_on_character_movement() called but supplied process name [" .. tostring(process_name) .. "] is not a string");
		return false;
	end;

	if not is_number(char_cqi) then
		ModLog("ERROR: notify_on_character_movement() called but supplied character cqi [" .. tostring(char_cqi) .. "] is not a number");
		return false;
	end;
	...
	if not character then
		ModLog("ERROR: notify_on_character_movement() called but no character with the supplied cqi [" .. tostring(char_cqi) .. "] could be found");
		return false;
	end;
	
	if not is_function(callback) then
		ModLog("ERROR: notify_on_character_movement() called but supplied callback [" .. tostring(callback) .. "] is not a function");
		return false;
	end;
...
function campaign_manager:has_mission_fired_for_faction(faction_key, mission_key)
	local event_generator = self:query_model():event_generator_interface();
	local q_faction = self:query_faction(faction_key);
...
function campaign_manager:register_mission_manager(mission_manager)
	if not is_missionmanager(mission_manager) then
		ModLog("ERROR: register_mission_manager() called but supplied mission manager [" .. tostring(mission_manager) .. "] is not a mission manager");
		return false;
	end;

	if self:get_mission_manager(mission_manager.mission_key) then
		ModLog("ERROR: register_mission_manager() called but supplied mission manager with key [" .. mission_manager.mission_key .. "] is already registered");
		return false;
	end;
...
function campaign_manager:mission_managers_from_string(str)
	...
	while true do
		...
		if not next_separator then
			ModLog("ERROR: mission_managers_from_string() called but supplied string is malformed: " .. str);
			return false;
		end;
		...
		elseif started == "false" then
			...
			ModLog("ERROR: mission_managers_from_string() called but parsing failed, boolean flag [" .. tostring(started) .. "] couldn't be decyphered, string is " .. str);
		end;
		...
		elseif completed == "false" then
			...
			ModLog("ERROR: mission_managers_from_string() called but parsing failed, boolean flag [" .. tostring(completed) .. "] couldn't be decyphered, string is " .. str);
			return false;
		end;
		...
		if not mission_manager then
			ModLog("ERROR:  mission_managers_from_string() is attempting to set up a mission with key [" .. tostring(mission_manager) .. "] but it isn't registered. All missions should be registered before the first tick.");
			return false;
		end;
...
function campaign_manager:add_turn_countdown_event(faction_name, turn_offset, event_name, context_str)
	if not is_string(faction_name) then
		ModLog("ERROR: add_turn_countdown_event() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:add_absolute_turn_countdown_event(faction_key, turn_to_trigger, event_name, context_str)
	if not is_string(faction_key) then
		ModLog("ERROR: add_absolute_turn_countdown_event() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
	
	if not is_number(turn_to_trigger) then
		ModLog("ERROR: add_absolute_turn_countdown_event() called but supplied trigger turn [" .. tostring(turn_to_trigger) .. "] is not a number");
		return false;
	end;
	
	if not is_string(event_name) then
		ModLog("ERROR: add_absolute_turn_countdown_event() called but supplied event name [" .. tostring(event_name) .. "] is not a string");
		return false;
	end;
	...
	if not is_string(context_str) then
		ModLog("ERROR: add_absolute_turn_countdown_event() called but supplied context string [" .. tostring(context_str) .. "] is not a string or nil");
		return false;
	end;
...
function campaign_manager:check_turn_countdown_events(faction_name)
	...
	if not is_table(turn_countdown_events) then
		ModLog("WARNING: check_turn_countdown_events() called but could not find a table corresponding to given faction name [" .. faction_name .. "], how can this be?");
		return false;
	end;
...
function campaign_manager:turn_countdown_events_from_string(state_str)
	...
	while true do
		...
		if not next_separator then
			ModLog("ERROR: turn_countdowns_from_string() called but supplied string is malformed: " .. state_str);
			return false;
		end;
		...
		if not turn_to_trigger then
			ModLog("ERROR: turn_countdowns_from_string() called but parsing failed, turns remaining number [" .. tostring(turn_to_trigger_str) .. "] couldn't be decyphered, string is " .. state_str);
			return false;
		end;
...
function campaign_manager:set_turn_number_modifier(modifier)
	if not is_number(modifier) or math.floor(modifier) ~= modifier then
		ModLog("ERROR: set_turn_number_modifier() called but supplied modifier [" .. tostring(modifier) .. "] is not an integer");
		return false;
	end;
...
function campaign_manager:start_custom_event_generator(event, condition, target_event, context_data_generator)
	if not is_string(event) then
		ModLog("ERROR: add_custom_event_generator() called but supplied event [" .. tostring(event) .. "] is not a string");
		return false;
	end;
	
	if not is_function(condition) then
		ModLog("ERROR: add_custom_event_generator() called but supplied condition [" .. tostring(condition) .. "] is not a function");
		return false;
	end;
	
	if not is_string(target_event) then
		ModLog("ERROR: add_custom_event_generator() called but supplied target_event [" .. tostring(target_event) .. "] is not a string");
		return false;
	end;
	
	if not is_function(context_data_generator) and not is_nil(context_data_generator) then
		ModLog("ERROR: add_custom_event_generator() called but supplied context data generator [" .. tostring(target_event) .. "] is not a function or nil");
		return false;
	end;
...
function campaign_manager:start_faction_region_change_monitor(faction_name)
	...
	if not is_string(faction_name) then
		ModLog("ERROR: start_faction_region_change_monitor() called but supplied name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
	...
	if self:get_faction_currently_processing() == faction_name then
		...
		for i = 1, #cached_region_list do
			...
			if not self:region_exists(current_cached_region) then		
				ModLog("WARNING: faction_region_change_monitor_validate_on_load() called but couldn't find region corresponding to key [" .. current_cached_region .. "] stored in cached region list - regenerating cached list");
				self:faction_region_change_monitor_process_turn_end(get_faction(faction_name));
				return;
			end;
		end;
...
function campaign_manager:stop_faction_region_change_monitor(faction_name)
	if not is_string(faction_name) then
		ModLog("ERROR: stop_faction_region_change_monitor() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:faction_region_change_monitor_process_turn_start(faction)
	...
	for i = 1, #cached_region_list do
		...
		if self:region_exists(current_cached_region) then		
			...
			ModLog("WARNING: faction_region_change_monitor_process_turn_start() called but couldn't find region corresponding to key [" .. current_cached_region .. "] stored in cached region list - discarding cached list and using current");
...
function campaign_manager:faction_region_change_monitor_from_str(str)
	...
	while true do
		...
		if not next_separator then	
			ModLog("ERROR: faction_region_change_monitor_from_str() called but supplied string is malformed: " .. str);
			return false;
		end;
		...
		if string.len(faction_str) == 0 then
			ModLog("ERROR: faction_region_change_monitor_from_str() called but supplied string contains a zero-length faction record: " .. str);
			return false;
		end;
...
function campaign_manager:load_exported_files(filename, path_str)
	...
	if not is_string(filename) then
		ModLog("ERROR: load_exported_files() called but no string filename supplied");
		return false;
	end;
	
	if path_str and not is_string(path_str) then
		ModLog("ERROR: load_exported_files() called but supplied path [" .. tostring(path_str) .. "] is not a string");
		return false;
	end;
	...
	if not is_string(all_files_str) or string.len(all_files_str) == 0 then
		ModLog("WARNING: load_exported_files() couldn't find any files with supplied name " .. filename);
		return;
	end;
...
function campaign_manager:cache_pending_battle_character(list, character)
	...
	if character:has_military_force() then
		...
		ModLog("WARNING: cache_pending_battle_character() called but supplied character (cqi: [" .. character:cqi() .. "], faction name: [" .. character:faction():name() .. "]) has no military force, how can this be? Not going to add CQI.");
		return;
	end;
...
function campaign_manager:pending_battle_cache_table_from_string(str)
	...
	while true do
		...
		if not cqi then
			ModLog("ERROR: pending_battle_cache_table_from_string() could not convert character cqi string [" .. tostring(cqi_str) .. "] into a number, inserting -1");
			cqi = -1;
		end;
		...
		if next_separator then
			...
			if not mf_cqi then
				ModLog("ERROR: pending_battle_cache_table_from_string() could not convert military force cqi string [" .. tostring(mf_cqi_str) .. "] into a number, inserting -1");
				cqi = -1;
			end;
...
function campaign_manager:pending_battle_cache_get_attacker(index)
	if not is_number(index) or index < 0 or index > #self.pending_battle_cached_attackers then
		ModLog("ERROR: pending_battle_cache_get_attacker() called but supplied index [" .. tostring(index) .. "] is out of range");
		return false;
	end;
...
function campaign_manager:pending_battle_cache_get_defender(index)
	if not is_number(index) or index < 0 or index > #self.pending_battle_cached_defenders then
		ModLog("ERROR: pending_battle_cache_get_defender() called but supplied index [" .. tostring(index) .. "] is out of range");
		return false;
	end;
...
function campaign_manager:pending_battle_cache_faction_was_attacker(faction_name)
	if not is_string(faction_name) then
		ModLog("ERROR: pending_battle_cache_faction_was_attacker() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:pending_battle_cache_faction_was_defender(faction_name)
	if not is_string(faction_name) then
		ModLog("ERROR: pending_battle_cache_faction_was_defender() called but supplied faction name [" .. tostring(faction_name) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:pending_battle_cache_get_enemies_of_char(character)
	if not is_character(character) then
		ModLog("ERROR: pending_battle_cache_get_enemies_of_character() called but supplied character [" .. tostring(character) .. "] is not a character");
		return false;
	end;
...
function campaign_manager:find_lowest_public_order_region_on_turn_start(faction_key)
	...
	if not is_string(faction_key) then
		ModLog("ERROR: find_lowest_public_order_region_on_turn_start() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: find_lowest_public_order_region_on_turn_start() called but no faction with supplied name [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:generate_region_rebels_event_for_faction(faction_key)
	...
	if not is_string(faction_key) then
		ModLog("ERROR: generate_region_rebels_event_for_faction() called but supplied faction key [" .. tostring(faction_key) .. "] is not a string");
		return false;
	end;
	
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: generate_region_rebels_event_for_faction() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:show_message_event(faction_key, title_loc_key, primary_detail_loc_key, secondary_detail_loc_key, is_persistent, index_num, end_callback, delay, suppress_whitelist)	
	if not is_string(title_loc_key) then
		ModLog("ERROR: show_message_event() called but supplied title localisation key [" .. tostring(title_loc_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(primary_detail_loc_key) then
		ModLog("ERROR: show_message_event() called but supplied primary detail localisation key [" .. tostring(primary_detail_loc_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(secondary_detail_loc_key) then
		ModLog("ERROR: show_message_event() called but supplied secondary detail localisation key [" .. tostring(secondary_detail_loc_key) .. "] is not a string");
		return false;
	end;
	
	if not is_boolean(is_persistent) then
		ModLog("ERROR: show_message_event() called but supplied is_persistent flag [" .. tostring(is_persistent) .. "] is not a boolean value");
		return false;
	end;
	
	if not is_number(index_num) then
		ModLog("ERROR: show_message_event() called but supplied index number [" .. tostring(index_num) .. "] is not a number");
		return false;
	end;
	
	if end_callback and not is_function(end_callback) then
		ModLog("ERROR: show_message_event() called but supplied end_callback [" .. tostring(end_callback) .. "] is not a function or nil");
		return false;
	end;
	
	if delay and not is_number(delay) then
		ModLog("ERROR: show_message_event() called but supplied end_callback [" .. tostring(delay) .. "] is not a number or nil");
		return false;
	end;
	...
	if not self:faction_exists(faction_key) then
		ModLog("ERROR: show_message_event_located() called but no faction with supplied key [" .. faction_key .. "] could be found");
		return false;
	end;
...
function campaign_manager:show_message_event_located(faction_key, title_loc_key, primary_detail_loc_key, secondary_detail_loc_key, x, y, is_persistent, index_num, end_callback, delay)	
	if not is_string(title_loc_key) then
		ModLog("ERROR: show_message_event_located() called but supplied title localisation key [" .. tostring(title_loc_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(primary_detail_loc_key) then
		ModLog("ERROR: show_message_event_located() called but supplied primary detail localisation key [" .. tostring(primary_detail_loc_key) .. "] is not a string");
		return false;
	end;
	
	if not is_string(secondary_detail_loc_key) then
		ModLog("ERROR: show_message_event_located() called but supplied secondary detail localisation key [" .. tostring(secondary_detail_loc_key) .. "] is not a string");
		return false;
	end;
	
	if not is_number(x) then
		ModLog("ERROR: show_message_event_located() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a number");
		return false;
	end;
	
	if not is_number(y) then
		ModLog("ERROR: show_message_event_located() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a number");
		return false;
	end;
	
	if not is_boolean(is_persistent) then
		ModLog("ERROR: show_message_event_located() called but supplied is_persistent flag [" .. tostring(is_persistent) .. "] is not a boolean value");
		return false;
	end;
	
	if not is_number(index_num) then
		ModLog("ERROR: show_message_event_located() called but supplied index_num [" .. tostring(index_num) .. "] is not a number");
		return false;
	end;
	
	if end_callback and not is_function(end_callback) then
		ModLog("ERROR: show_message_event_located() called but supplied end_callback [" .. tostring(end_callback) .. "] is not a function or nil");
		return false;
	end;
	
	if delay and not is_number(delay) then
		ModLog("ERROR: show_message_event_located() called but supplied end_callback [" .. tostring(delay) .. "] is not a number or nil");
		return false;
	end;
...
function campaign_manager:show_subtitle(key, full_key_supplied, should_force)
	...
	if not is_string(key) then
		ModLog("ERROR: show_subtitle() called but supplied key [" .. tostring(key) .. "] is not a string");
		return false;
	end;
	...
	if not is_string(localised_text) then
		ModLog("ERROR: show_subtitle() called but could not find any localised text corresponding with supplied key [" .. tostring(key) .. "] in scripted_subtitles table");
		return false;
	end;
	...
	if not uic_subtitles then
		ModLog("ERROR: show_subtitles() could not find the scripted_subtitles uicomponent");
		return false;
	end;
...
function campaign_manager:show_benchmark_if_required(non_benchmark_callback, cindy_str, duration, start_x, start_y, start_d, start_b, start_h)
	...
	if not is_function(non_benchmark_callback) then
		ModLog("ERROR: show_benchmark_if_required() called but supplied callback [" .. tostring(non_benchmark_callback) .. "] is not a function");
		return false;
	end;
	
	if not is_string(cindy_str) then
		ModLog("ERROR: show_benchmark_if_required() called but supplied cindy path [" .. tostring(cindy_str) .. "] is not a string");
		return false;
	end;
	
	if not is_number(duration) or duration <= 0 then
		ModLog("ERROR: show_benchmark_if_required() called but supplied duration [" .. tostring(duration) .. "] is not a number greater than zero");
		return false;
	end;
	
	if not is_number(start_x) or start_x <= 0 then
		ModLog("ERROR: show_benchmark_if_required() called but supplied start x co-ordinate [" .. tostring(start_x) .. "] is not a number greater than zero");
		return false;
	end;
	
	if not is_number(start_y) or start_y <= 0 then
		ModLog("ERROR: show_benchmark_if_required() called but supplied start y co-ordinate [" .. tostring(start_y) .. "] is not a number greater than zero");
		return false;
	end;
	
	if not is_number(start_d) or start_d <= 0 then
		ModLog("ERROR: show_benchmark_if_required() called but supplied start camera distance [" .. tostring(start_d) .. "] is not a number greater than zero");
		return false;
	end;
	
	if not is_number(start_b) then
		ModLog("ERROR: show_benchmark_if_required() called but supplied start camera bearing [" .. tostring(start_b) .. "] is not a number");
		return false;
	end;
	
	if not is_number(start_h) or start_h <= 0 then
		ModLog("ERROR: show_benchmark_if_required() called but supplied start camera height [" .. tostring(start_h) .. "] is not a number greater than zero");
		return false;
	end;
...
function campaign_manager:register_cutscene(cutscene)
	if not is_campaigncutscene(cutscene) then
		ModLog("ERROR: register_cutscene() called but supplied object [" .. tostring(cutscene) .. "] is not a campaign cutscene");
		return false;
	end;
...
function campaign_manager:steal_key_with_callback(name, key, callback)
	if not is_string(name) then
		ModLog("ERROR: steal_key_with_callback() called but supplied name [" .. tostring(name) .. "] is not a string");
		return false;
	end;

	if not is_string(key) then
		ModLog("ERROR: steal_key_with_callback() called but supplied key [" .. tostring(key) .. "] is not a string");
		return false;
	end;
	...
	for i = 1, #key_steal_entries_for_key do
		if key_steal_entries_for_key[i].name == name then
			ModLog("ERROR: steal_key_with_callback() called but a steal entry with supplied name [" .. name .. "] already exists for supplied key [" .. tostring(key) .. "]");
			return false;
		end;
	end;
...
function campaign_manager:release_key_with_callback(name, key)
	if not is_string(name) then
		ModLog("ERROR: release_key_with_callback() called but supplied name [" .. tostring(name) .. "] is not a string");
		return false;
	end;

	if not is_string(key) then
		ModLog("ERROR: release_key_with_callback() called but supplied key [" .. tostring(key) .. "] is not a string");
		return false;
	end;
...
function campaign_manager:get_closest_character_to_position_from_faction(faction, x, y, generals_only, consider_garrison_commanders)
	...
	if not faction_found then
		ModLog("ERROR: get_closest_character_to_position_from_faction() called but supplied faction [" .. tostring(faction) .. "] is not a valid faction, or a string name of a faction");
		return false;
	end;
	
	if not is_number(x) or x < 0 then
		ModLog("ERROR: get_closest_character_to_position_from_faction() called but supplied x co-ordinate [" .. tostring(x) .. "] is not a positive number");
		return false;
	end;
	
	if not is_number(y) or y < 0 then
		ModLog("ERROR: get_closest_character_to_position_from_faction() called but supplied y co-ordinate [" .. tostring(y) .. "] is not a positive number");
		return false;
	end;
...
-- diplomacy
-----------------------------------------------------------------------------
function campaign_manager:force_declare_war(faction_key, target_faction_key, is_silent)
	if self:is_at_war_with(faction_key, target_faction_key) then
		ModLog("ERROR: Factions already at war!");
...
function campaign_manager:grant_military_access(faction_key, target_faction_key, is_silent)
	if self:has_specified_diplomatic_deal(faction_key, target_faction_key, "treaty_components_military_access") then
		ModLog("ERROR: Factions already have military access.");
...
function campaign_manager:apply_automatic_deal_between_factions(faction_key, target_faction_key, deal_key, is_silent)
	...
	if faction_key == target_faction_key then
		ModLog("ERROR: Cannot apply deal with self!");
		return false;
	end;

	if not is_string(deal_key) then
		ModLog("ERROR: Deal key must be a string.");
		return;
	end;

	if not faction or faction:is_null_interface() then
		ModLog("ERROR: Passed in faction doesn't exist.");
		return;
	end;

	if not target_faction or target_faction:is_null_interface() then
		ModLog("ERROR: Passed in target_faction doesn't exist.");
		return;
	end;

	if not faction:can_apply_automatic_diplomatic_deal(deal_key, target_faction, "faction_key:" .. target_faction:name()) then
		ModLog("ERROR: Cannot apply deal between factions."..tostring(deal_key).."  "..tostring(faction_key).."  "..tostring(target_faction_key));
		return false;
	else
		ModLog("Will apply deal:"..tostring(deal_key).."  "..tostring(faction_key).."  "..tostring(target_faction_key));
	end;

	if is_silent then
		output("blocking EF")
		self:disable_event_feed_events(true, "3k_event_category_diplomacy");
		core:add_listener(
			"ev_supp",
			...
			function()
				...
				function()
					...
					self:disable_event_feed_events(false, "3k_event_category_diplomacy");
				end,
				false
			);
			end,
	...
	self:modify_faction(faction):apply_automatic_diplomatic_deal(
		deal_key, 
		target_faction,
		"faction_key:" .. target_faction:name()
	);

	return true;
end;

function campaign_manager:is_at_war_with(faction_key, target_key)
	return self:has_specified_diplomatic_deal(faction_key, target_key, "treaty_components_war");
end;

function campaign_manager:is_allied_to(faction_key, target_key)
	return self:has_specified_diplomatic_deal(faction_key, target_key, "treaty_components_alliance");
end;

function campaign_manager:has_specified_diplomatic_deal(faction_key, target_key, deal_key)
	local faction = cm:query_faction(faction_key);
...
function campaign_manager:factions_at_war_with(faction_key)
	local faction = cm:query_faction(faction_key);
	...
	return faction:factions_we_have_specified_diplomatic_deal_with("treaty_components_war");
end;

function campaign_manager:force_confederation(confederator_key, confederated_key)
	
	if not is_string(confederator_key) then
        ModLog("ERROR: liu_bei_ai_confederation() called but supplied faction key [" .. tostring(confederator_key) .. "] is not a string")
        return false;
	end;
	
	if not is_string(confederated_key) then
        ModLog("ERROR: liu_bei_ai_confederation() called but supplied other_faction key [" .. tostring(confederated_key) .. "] is not a string")
        return false;
	end;	
	...
	if confederated:is_human() then
		output("### Not proceeding with confederation as local faction is either " .. confederator_key .. " or " .. confederated_key .. " ###")
		return false;
	end;

	if confederator:is_dead() or confederated:is_dead() then
		output("### Either " .. confederator_key .. " or " .. confederated_key .. " is dead ###")
		return false;
	end;

	if cm:modify_faction(confederator):make_confederation_with(confederated) then
		output("### Successfully to trigger confederation with " .. confederator_key .. " and " .. confederated_key .. " ###")
	else
		output("### Failed to trigger confederation with " .. confederator_key .. " and " .. confederated_key .. " ###")
	end;
...
function campaign_manager:world_power_token_exists(token_key)
	if not self:get_world_power_token_owner(token_key) then
		return false;
	end;
...
--- @return bool is it owned by them?
...
function campaign_manager:disable_regional_pooled_resource(pooled_resource_key, optional_region)
	...
	if not is_string(pooled_resource_key) then
		ModLog("ERROR: pooled_resource_key is not a string!");
		return false;
	end;

	if optional_region and not is_string(optional_region) then
		ModLog("ERROR: optional_region is not a string!");
		return false;
	end;
	...
	local function disable_resource(resource_key, modify_region)
		if not modify_region or modify_region:is_null_interface() then
			ModLog("ERROR: optional_region could not be found! [" .. tostring(modify_region:query_region():name()) .. "]");
			return false;
		end;
		...
		if not q_resource or q_resource:is_null_interface() then
			--ModLog("ERROR: pooled_resource_key could not be found! Region [" .. tostring(modify_region:query_region():name()) .. "], Resource [" .. tostring(pooled_resource_key) .. "]"); -- disabled due to spam
			return false;
		end;

		if not q_resource:enabled() then
			ModLog("ERROR: disabling a pooled resource which is already disabled!");
			return false;
		end;
...
function campaign_manager:enable_regional_pooled_resource(pooled_resource_key, optional_region)
	...
	local function enable_resource(resource_key, modify_region)
		...
		if not q_resource or q_resource:is_null_interface() then
			--ModLog("ERROR: pooled_resource_key could not be found! Region [" .. tostring(modify_region:query_region():name()) .. "], Resource [" .. tostring(pooled_resource_key) .. "]");
			return false;
		end;

		if q_resource:enabled() then
			ModLog("ERROR: enabling a pooled resource which is already enabled!");
			return false;
		end;
...
function campaign_manager:get_regional_pooled_resource(pooled_resource_key, region)
	...
	if not is_string(region) then
		ModLog("ERROR: region is not a string!");
		return false;
	end;
	...
	if not m_region or m_region:is_null_interface() then
		ModLog("ERROR: optional_region could not be found! [" .. tostring(region) .. "]");
		return false;
	end;
	...
	if not q_resource or q_resource:is_null_interface() then
		--ModLog("ERROR: pooled_resource_key could not be found! Region [" .. tostring(region) .. "], Resource [" .. tostring(pooled_resource_key) .. "]");
		return false;
	end;
...
function campaign_manager:disable_world_pooled_resource(pooled_resource_key)
	...
	if not q_resource or q_resource:is_null_interface() then
		ModLog("ERROR: pooled_resource_key could not be found! Resource [" .. tostring(pooled_resource_key) .. "]");
		return false;
	end;

	if not q_resource:enabled() then
		ModLog("ERROR: disabling a pooled resource which is already disabling!");
		return false;
	end;
...
function campaign_manager:disable_faction_pooled_resource(pooled_resource_key, faction_key)
	...
	if not is_string(faction_key) then
		ModLog("ERROR: faction_key is not a string!");
		return false;
	end;

	local q_faction = cm:query_faction(faction_key);

	if not q_faction then
		return false;
	end

	local q_resource = q_faction:pooled_resources():resource(pooled_resource_key);
	...
	self:modify_model():get_modify_pooled_resource(q_resource):diable();
end;
...
function campaign_manager:teleport_character(query_character, coordinate_x, coordinate_y)
	...
	if not query_character or query_character:is_null_interface() then
		ModLog("ERROR: teleport_character Could not find a valid characters.");
		return false;
	end;
