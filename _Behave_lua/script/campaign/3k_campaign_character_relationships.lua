...
character_relationships.character_spawn_max_relationship_targets = 30;
character_relationships.character_spawn_relationship_chance = 95; -- chance of a relationship forming on spawn.
...
function character_relationships:character_spawn_get_target_character() -- return query_character interface
	...
	for i=1, #self.character_spawn_relationship_targets do
		...
		if --[[ target_character:faction():is_human() or --]] not target_character:character_type("general") or target_character:is_dead() then 
			...
			--ModLog("(Havie) I manually changed this to stop removing human character relationships target= "..tostring(target_character:generation_template_key()))
